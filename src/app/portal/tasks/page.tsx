...
function PortalPageContent() {
  ...
  // Only show assigned tasks unless user has "tasks.view_all" permission
  if (user?._id && !user?.isAdmin && !can("tasks.view_all")) baseParams.assignee = user._id;
  ...
  // Stats (scoped to this user + project, unless they can view all)
  ...
  const handleCreateTask = async (e: React.FormEvent) => {
    ...
    setCreating(true);
    try {
      const folderId = createFolder || activeFolder;
      if (!folderId) {
  ...
  const handleInlineSelect = async (id: string, field: string, value: string) => {
    try {
      const res = await fetch(`/api/tasks/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="space-y-6">
      ...
      {!can("tasks.view") ? (
        <div className="card p-16 text-center">
          <p className="text-gray-400 text-sm">You don't have permission to view tasks. Contact your admin.</p>
        </div>
      ) : !activeFolder ? (
        isLoading ? (
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Due Date / Within</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Attachments</th>
                  </tr>
                </thead>
                  ...
                  {tasks.map((task) => (
                    <tr key={task._id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        {can("tasks.edit") && editingCell?.id === task._id && editingCell?.field === "title" ? (
                          <input
                            type="text"
                          ...
                          <p
                            className={cn("text-sm font-medium text-gray-900", can("tasks.edit") && "cursor-pointer hover:text-brand")}
                            onClick={() => can("tasks.edit") && startEditing(task._id, "title", task.title)}
                          >
                            {task.title}
                          </p>
                        )}
                        {task.description && !editingCell?.field && (
      ...
      {/* Create Task Modal */}
      {showCreateModal && can("tasks.create") && (
        <Portal><div
